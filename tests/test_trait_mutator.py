...
from spark_character.chip_loader import _coerce_yaml_dict
import pytest
import yaml

@pytest.fixture
def founder_operator_chip():
    VALID_CHIP_YAML = """
...
safety:
  harm_avoidance:
    - do not flatter
"""
    return _coerce_yaml_dict(yaml.safe_load(VALID_CHIP_YAML))
...
def test_apply_deltas_clamps_to_unit(founder_operator_chip) -> None:
    chip = founder_operator_chip
...
def test_chip_to_yaml_dict_round_trip_preserves_structure(founder_operator_chip) -> None:
    chip = founder_operator_chip
...
def test_chip_to_yaml_dict_after_mutation_reflects_new_values(founder_operator_chip) -> None:
    chip = founder_operator_chip
    new_chip = _apply_deltas(
        chip,
